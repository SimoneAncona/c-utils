...
#define __if_null_to_string__(x) \
	if (x == NULL)               \
 ...
 * @since v1.2
 */
byte_vector_t str_split(string_t);
...
#define __str_pool_check_for_init__() \
	if (__str_pool__ == NULL)         \
	{                                 \
		__str_pool_alloc__();         \
	}
#define __str_pool_alloc__()                                                                             \
	__str_pool__ = (__str_ref_info_t__ *)malloc(sizeof(__str_ref_info_t__) * __INITAL_STR_POOL_SIZE___); \
	for (size_t i = 0; i < __INITAL_STR_POOL_SIZE___; i++)                                               \
	{                                                                                                    \
		__str_pool__[i].str = NULL;                                                                      \
		__str_pool__[i].refs = 0;                                                                        \
	}                                                                                                    \
	__str_pool__[__NULL_STRING_POOL_INDEX__].str = __str_null_string__()
#define __str_pool_extend__()                                                                                                                \
	__str_pool__ = (__str_ref_info_t__ *)realloc(__str_pool__, sizeof(__str_ref_info_t__) * (__str_pool_size__ + __STR_POOL_BLOCK_SIZE___)); \
	__str_pool_size__ += __STR_POOL_BLOCK_SIZE___
...
string_t str_substr(string_t s, size_t start, size_t end)
{
	...
	if (end <= start || end > s_len || start > s_len)
...
string_t num_to_string(number_t n)
{
	...
	string_t res = str_new_string("");
	if (n->__n__->__len__ == 0)
		return str_new_string("NaN");
	char c[2] = {0, 0};
	c[0] = n->__n__->__v__[0] + '0';
	res = str_concat(res, c);
	res = str_concat(res, ".");
	if (n->__n__->__len__ == 1)
	{
		res = str_concat(res, "0E");
		res = str_concat(res, str_from_int(n->__e__));
		return res;
	}
	for (size_t i = 1; i < n->__n__->__len__; i++)
	{
		c[0] = n->__n__->__v__[i] + '0';
		res = str_concat(res, c);
	}
	res = str_concat(res, "0E");
	res = str_concat(res, str_from_int(n->__e__));
...
bool str_ends_with(string_t main_str, string_t sub_str)
{
	...
	{
		if (main_str[i] != sub_str[i - s1_len + s2_len])
			return false;
	}
